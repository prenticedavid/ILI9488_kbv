...
//
// >>>>>>> not working on ST Core or ESP32.   Needs Transactions.
...
#if defined(ESP32)
#define SD_CS_PAR  5    //D10 is /CS on parallel MCUFRIEND
#define SD_CS_SPI 17    //D4  is /CS on SPI displays
#elif defined(ESP8266)
#define SD_CS_SPI D4    //D4  is /CS on SPI displays
#else
#define SD_CS_PAR 10    //D10 is /CS on parallel MCUFRIEND
#define SD_CS_SPI  4    //D4  is /CS on SPI displays
#endif
#if MCUFRIEND_KBV_H_ > 100 || defined(ESP32_PARALLEL)
#define SD_CS     SD_CS_PAR
#else
#define SD_CS     SD_CS_SPI
#endif
    ...
    if (bmpID != 0x4D42) ret = 2; // bad ID
        ...
        for (row = 0; row < h; row++) { // For each scanline...
            ...
            for (col = 0; col < w; ) {  //pixels in row
                ...
                // regular array of uin16_t pixels
#if defined(_TFT_eSPIH_)
                tft.pushColors(lcdbuffer, lcdidx, true);
#else
                tft.pushColors(lcdbuffer, lcdidx, first); //.kbv
#endif
                first = false;
                col += lcdidx;
            }           // end cols
